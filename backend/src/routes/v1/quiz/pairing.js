...
router.get('/results', async (req, res) => {
  ...
  if (!sessionId) {
    console.warn('[Pairing] Missing session ID in query param.');
    return res.status(400).json({ error: 'Missing sessionId in query param (s)' });
  }

  try {
    console.log(`[Pairing] Fetching session: ${sessionId}`);
    ...
    if (error) {
      console.error('[Pairing] Supabase fetch error:', error);
      return res.status(500).json({ error: 'Failed to load session from database' });
    }

    if (!session) {
      console.warn(`[Pairing] No session found for ID: ${sessionId}`);
    ...
    if (!movie && !recipe) {
      console.info(`[Pairing] No match found for session ${sessionId}`);
      return res.status(200).json({
        message: 'No results matched your preferences, please try a different quiz combination.',
        movie: [],
        recipe: [],
      });
    }
...
async function fetchMovieRecommendation(answers) {
  ...
  if (error) {
    console.warn('[Movie Fetch] Supabase error:', error);
    return null;
  }

  if (!data?.length) {
    console.info('[Movie Fetch] No movie matched filters.');
    return null;
  }
...
async function fetchRecipeRecommendation(answers) {
  ...
  const { data, error } = await q;
  if (error) {
    console.warn('[Recipe Fetch] Supabase error:', error);
    return null;
  }

  if (!data?.length) {
    console.info('[Recipe Fetch] No recipe matched filters.');
    return null;
  }
